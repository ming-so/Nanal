...
class CalendarAppWidgetModel {
    ...
    public CalendarAppWidgetModel(Context context, String timeZone) {
        ...
        time.setToNow(); // This is needed for gmtoff to be set. gmtoff가 설정되기 위해 필요함
    ...
    public void buildFromCursor(Cursor cursor, String timeZone) {
        ...
        while (cursor.moveToNext()) {
            ...
            // 직접 계산하지 않는 이유는 종일 이벤트들의 잘못된 endDay를 정하는 것과 같음
            ...
            // 종일 이벤트를 처리하기 위해 쿼리할 때, 몇 가지 추가 이벤트를 얻을 수 있음
            if (end < mNow) {
                continue;
            }
    ...
    private EventInfo populateEventInfo(long eventId, boolean allDay, long start, long end,
        ...
        // 이벤트 시작 시간에 대해 사람이 읽을 수 있는 문자열을 가져옴
     ...
     * RowInfo는 위젯에서 단일 행을 나타내는 클래스
     * 위젯의 행은 일 헤더 또는 이벤트일 수 있기에 DayInfo 또는 EventInfo 인스턴스의 포인터만 있음
     */
    static class RowInfo {
         ...
         * mType: 일 헤더(TYPE_DAY)인지 이벤트(TYPE_MEETING)인지
         */
        final int mType;
         ...
         * infos.
         * mType이 TYPE_DAY라면, mData는 일 정보의 인덱스임
         * mTyoe이 TYPE_MEETING이라면, mData는 이벤트 정보의 인덱스임
         */
        final int mIndex;
     ...
     * EventInfo는 위젯에서 이벤트를 나타내는 클래스
     * 적절한 지역화된 문자열 및 가시성(visibility) 설정을 포함하여,
     * 이벤트를 표시하는 데 필요한 모든 데이터를 포함하고 있음
     */
     ...
     * DayInfo는 위젯에서 일 헤더를 나타내는 클래스
     * 적절한 지역화된 문자열을 포함하여,
     * 그날의 헤더를 표시하는 데 필요한 모든 데이터를 포함하고 있음
     */
