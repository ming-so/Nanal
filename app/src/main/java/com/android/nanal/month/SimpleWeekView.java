 ...
 * 일주일을 그리는 다이나믹 view
 * 주 번호를 표시하거나, 지정된 요일에 주를 시작하거나, 일수를 줄이도록 구성 가능
 * ListView 내에서 단일 뷰로 사용하기 위한 것임
 * </p>
 */
public class SimpleWeekView extends View {
     ...
     * 이 매개변수들은 어떻게 나타나는지를 컨트롤하기 위해 view에 전달될 수 있음
     * 기본값이 대부분의 레이아웃에 정확하게 맞지 않을 가능성이 낮지만
     * VIEW_PARAMS_WEEK는 유일한 필수 필드임
     */
    /**
     * This specifies the position (or weeks since the epoch) of this week,
     * calculated using {@link Utils#getWeeksSinceEpochFromJulianDay}
     * 이 값은 Utils#WeeksSinceEpochFromJulianDay를 사용하여 계산한
     * 이번 주의 위치를 지정함
     */
     ...
     * 일요일부터 토요일 중 이 view에서 선택한 날짜를 설정함
     */
    ...
    // is)
    // 시간/날짜를 표시할 시간대(오늘이 언제인지 확인하는 데 사용)
     ...
     * 이번 주에 표시할 모든 매개 변수를 설정함
     * 필요한 매개 변수는 주 번호뿐임
     * 다른 매개 변수들은 기본값을 가지며, 포커스된 월을 제외하고 새로운 값을 포함하는 경우에만 업데이트되고,
     * 값이 전달되지 않았을 때 포커스되지 않은 달을 항상 디폴트로 하는... 이 경우의 포커스된 월을 제외함
     *
    ...
    public void setWeekParams(HashMap<String, Integer> params, String tz) {
        ...
        // new month undo it
        // 반복이 끝날 때 한 번 더 추가함, 만약 새로운 달로 간다면 취소함?
        if (time.monthDay == 1) {
            time.monthDay--;
            time.normalize(true);
        }
     ...
     * 지정된 x 위치에 있는 요일을 계산해서 주 번호를 구함
     * 날짜를 참조한...? 시간(Time) 또는 null을 반환함
     *
     * @param x The x position of the touch event
     *           터치 이벤트의 x 좌표
     * @return A time object for the tapped day or null if the position wasn't
     *         in a day
     *         탭된 날짜에 대한 time 객체 또는 해당 위치가 하루 내에 없는 경우 null
     *
     */
    ...
    public boolean onHoverEvent(MotionEvent event) {
        ...
        // on.
        // 접근성 및 탐사..가 켜져 있는 경우에만 경우에만 접근성 이벤트를 전송함
