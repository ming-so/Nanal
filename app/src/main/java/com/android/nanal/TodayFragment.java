...
public class TodayFragment extends Fragment implements CalendarController.EventHandler, ViewSwitcher.ViewFactory {
    ...
    public TodayFragment(long timeMillis) {
        if (timeMillis == 0) {
            mSelectedDay.setToNow();
        } else {
    ...
    public View makeView() {
        ...
        DayView view = new DayView(getActivity(), CalendarController
                .getInstance(getActivity()), mViewSwitcher, mEventLoader, 7);
        view.setId(VIEW_ID);
