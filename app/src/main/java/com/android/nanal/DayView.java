 ...
 * 다중일 view, 지금까지 테스트된 날짜는 1일과 7일뿐임?
 */
        ...
        ScaleGestureDetector.OnScaleGestureListener, View.OnClickListener, View.OnLongClickListener
{
    ...
    /* package */ public static final int MINUTES_PER_DAY = MINUTES_PER_HOUR * 24;
    ...
    // 많은 allday 이벤트 텍스트에 fade 애니메이션하는 시간
    ...
    // 창이 열려 있는 경우 5분마다 현재 시간 line 업데이트
     ...
     * 종일 이벤트의 최소한 요구되는 높이임
     * 확장되지 않은 경우, 종일 이벤트에 이 높이가 사용됨
     * 종일 이벤트가 확장된 경우, 이 높이에 있는 모든 이벤트에 맞게 커지려고 시도함
     */
     ...
     * 정규 이벤트를 표시하기 위해 예약된 최소 크기임
     * 확장된 종일은 이 영역으로 확장될 수 없음
     */
    ...
    // 보다 유용한 디버깅을 위해 패키지 내에서 이 기능을 볼 수 있게 함
    Time mBaseDate;
     ...
     * 캘린더 동기화 중, 동일한 이벤트에 대해 여러 개의 빠른 view를 표시하는 걸 방지하기 위해 사용함
     * 캘린더 시작할 때 또는 이벤트로 스크롤하여 이벤트를 선택할 때 유효함
     * 사용자가 빈 시간 슬롯으로 스크롤하거나, view를 변경하거나, 이벤트를 삭제하면 무효가 됨
     */
     ...
     * 이 변수는 가장 최근 이벤트 로딩에 사용된 시작 millis 매개변수를 추적해서
     * 불필요하게 이벤트를 다시 로드하는 것을 방지하는 데 도움이 됨
     * 만약 다음 번 재로드한 게 이것과 일치하면 이벤트가 재로드되지 않음
     * 재로드하려면 이 값을 0으로 설정하기(clearCachedEvents() 메소드에서 0으로 설정됨)
     */
    ...
    // 클릭된 이벤트에서 "클릭된" 색상을 지우고 이벤트를 시작함
