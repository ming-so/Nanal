...
public class TimeZoneData {
    ...
    void loadTzs(Context context) {
        ...
        for (TimeZoneInfo tz : mTimeZones) {
            ...
            // 검색시 GMT+xx:xx 스타일의 display name 전부 스킵
    ...
    private HashSet<String> loadTzsInZoneTab(Context context) {
         ...
         * 'backward' 파일에는 새 표준 시간대 ID와 이전 표준 시간대 ID 간의 매핑이 포함
         * 이전 것을 사용하지 않음
         */
        try {
            is = am.open("backward");
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (!line.startsWith("#") && line.length() > 0) {
                    // 0: "Link"        링크
                    // 1: New tz id     새 표준 시간대 ID
                    // Last: Old tz id  이전 표준 시간대 ID
         ...
         * 시간대와 국가 코드 목록이 포함
         * 국가별로 먼저 분류된 후, 국가 내에서 지리적인 타당성을 만들고,
         * 가장 인구가 많은 지역을 우선시하되, (1)과 모순되지 않음
         */
        try {
            ...
            while ((line = reader.readLine()) != null) {
                if (!line.startsWith("#")) { // Skip comment lines
                    // 0: country code      국가 코드
                    // 1: coordinates       좌표
                    // 2: time zone id      시간대 ID
                    // 3: comments          코멘트
                    ...
                    // 지정된 국가에서 시간대가 고유한 경우 시간대 리스트에 추가
