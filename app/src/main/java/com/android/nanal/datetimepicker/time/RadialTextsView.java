...
class RadialTextsView extends View {
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (!mDrawValuesReady) {
            ...
            if (!mIs24HourMode) {
                ...
                // AM/PM 원을 그려야 하기 때문에, 메인 원은 조금 더 높은 중심이 있어야 함
                // AM/PM 원의 반경을 위로 올림
            ...
            // 텍스트 위치가 정적이기 때문에, 애니메이션을 미리 렌더링함
            renderAnimations();
     ...
     * 삼각측량(?) 단위 원을 사용하여, 지정된 원 반지름을 기준으로 텍스트를 그려야 하는 위치 계산
     * 값을 textGridHeights와 textGridWidths 매개 변수에 배치
     */
    private void calculateGridSizes(float numbersRadius, float xCenter, float yCenter,
         ...
         * 이 숫자들은 7x7 그리드로 그려져야 함, 유닛 서클의 점들을 나타냄
         */
        ...
        // 텍스트의 베이스 라인을 위해 yTextBase를 약간 더 낮게 설정
     ...
     * textGrid 매개변수로 지정된 위치에 12개의 텍스트 값을 그림
     */
