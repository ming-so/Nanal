...
public class DiaryLoader {
    ...
    public DiaryLoader(Context context) {
        mContext = context;
     ...
     */
    public void loadEventsInBackground(final int numDays, final ArrayList<Diary> diaries,
        ...
        LoadDiariesRequest request = new LoadDiariesRequest(id, startDay, numDays,
                diaries, successCallback, cancelCallback);
    ...
    void loadEventDaysInBackground(int startDay, int numDays, boolean[] eventDays,
        ...
        // 백그라운드 스레드로 로드 요청 전송
        LoadDiaryDaysRequest request = new LoadDiaryDaysRequest(startDay, numDays,
    ...
    private static interface LoadRequest {
        ...
        public void skipRequest(DiaryLoader diaryLoader);
    }
     ...
     *
     */
    private static class LoadDiaryDaysRequest implements LoadRequest {
        /**
        ...
        public int diaryDay;
        public int numDays;
        public boolean[] diaryDays;
        public Runnable uiCallback;

        public LoadDiaryDaysRequest(int day, int numDays, boolean[] diaryDays,
                                    final Runnable uiCallback)
        {
            this.diaryDay = day;
        ...
        public void processRequest(DiaryLoader diaryLoader)
        {
            final Handler handler = diaryLoader.mHandler;
            ContentResolver cr = diaryLoader.mResolver;

            // Clear the event days
            // eventDays 지우기
            Arrays.fill(diaryDays, false);

            //query which days have events
            // 이벤트가 있는 날짜를 쿼리함
            Cursor cursor = EventDays.query(cr, diaryDay, numDays, PROJECTION);
            try {
                ...
                while (cursor.moveToNext()) {
                    ...
                    int firstIndex = Math.max(firstDay - diaryDay, 0);
                    int lastIndex = Math.min(lastDay - diaryDay, 30);
    ...
    private static class LoadDiariesRequest implements LoadRequest {
        ...
        public LoadDiariesRequest(int id, int startDay, int numDays, ArrayList<Diary> diaries,
        ...
        public void processRequest(DiaryLoader diaryLoader) {
            ...
            if (id == diaryLoader.mSequenceNumber.get()) {
                diaryLoader.mHandler.post(successCallback);
            } else {
                diaryLoader.mHandler.post(cancelCallback);
            }
        }
    ...
    private static class LoaderThread extends Thread {
        ...
        DiaryLoader mDiaryLoader;

        public LoaderThread(LinkedBlockingQueue<LoadRequest> queue, DiaryLoader diaryLoader) {
            mQueue = queue;
            mDiaryLoader = diaryLoader;
        }
            ...
            while (true) {
                try {
                    ...
                    while (!mQueue.isEmpty()) {
                        ...
                        // 건너뛴 요청을 알림
                        request.skipRequest(mDiaryLoader);
                    ...
                    request.processRequest(mDiaryLoader);
                } catch (InterruptedException ex) {
